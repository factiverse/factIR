...
class FaissSearch:
    ...
    def get_top_n_neighbours(self, query_vector: Tensor, top_k: int) -> Dict:
        ...
        assert (
            indices.shape == distances.shape == (query_vector.shape[0], top_k)
        )
    ...
    def load_index_if_available(self, dataset_name=None) -> None:
        if os.path.exists("indices/faiss/index_faiss_{}".format(dataset_name)):
            self.ann = faiss.read_index(
                "indices/faiss/index_faiss_{}".format(dataset_name)
            )
            return True
        else:
            return False
    ...
    def create_index(self, passage_vectors, dataset_name=None):
        ...
        faiss.write_index(
            self.ann, "indices/faiss/index_faiss_{}".format(dataset_name)
        )
