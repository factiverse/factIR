...
class HfRetriever:
    ...
    def __init__(self, config=DenseHyperParams) -> None:
        """Initialize huggingface based retrieval model.

        Args:
            config (_type_, optional): hyperparam config. Defaults to DenseHyperParams.
        """
        super().__init__()
        self.config = config
    ...
    def encode_queries(
        ...
        Args:
            queries (List[Claim]): Claims
            batch_size (int, optional): Defaults to 16.

        Returns:
            Union[List[Tensor], np.ndarray, Tensor]: embeddings of queries
        """
        with torch.no_grad():
    ...
    def retrieve_in_chunks(
        ...
        **kwargs,
        ...
        query_embeddings = self.encode_queries(
            queries, batch_size=self.batch_size
        )
    ...
    def retrieve(
        ...
        **kwargs,
        ...
        Returns:
            Dict[str, Dict[str, float]]: retrieval results
        """
        ...
        for idx, q in enumerate(queries):
            ...
            for index, id in enumerate(cos_scores_top_k_idx[idx]):
                response[str(q.idx)][str(corpus[id].idx)] = float(
