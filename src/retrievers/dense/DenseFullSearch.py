...
class DenseFullSearch:
    def __init__(self, config=DenseHyperParams) -> None:
        ...
        self.question_encoder = SentenceTransformer(
            self.config.query_encoder_path, device="cuda:1"
        )
        self.context_encoder = SentenceTransformer(
            self.config.document_encoder_path, device="cuda:1"
        )
    ...
    def encode_queries(
        self, queries: List[Claim], batch_size: int = 16, **kwargs
    ) -> Union[List[Tensor], np.ndarray, Tensor]:
        queries = [query.text for query in queries]
        return self.question_encoder.encode(
            queries, batch_size=batch_size, **kwargs
        )

    def encode_corpus(
        self, corpus: List[Evidence], **kwargs
    ) -> Union[List[Tensor], np.ndarray, Tensor]:
        """Encode the retrieval corpus

        Args:
            corpus (List[Evidence]): _description_

        Returns:
            Union[List[Tensor], np.ndarray, Tensor]: _description_
        """
        contexts = []
        ...
        return self.context_encoder.encode(
            contexts, batch_size=self.batch_size, **kwargs
        )
    ...
    def retrieve_in_chunks(
        ...
        **kwargs,
        ...
        corpus_ids = [str(doc.idx) for doc in corpus]
        ...
        query_ids = [str(query.idx) for query in queries]
        result_heaps = {
        ...
        **kwargs,
        ...
        for idx, q in enumerate(queries):
            ...
            for index, c_idx in enumerate(cos_scores_top_k_idx[idx]):
                document_id = str(corpus[c_idx].idx)
                response[str(q.idx)][document_id] = float(
